...
source /var/lib/akbarvpn/ipvps.conf
if [[ "$IP2" = "" ]]; then
...
until [[ $VPN_USER =~ ^[a-zA-Z0-9_]+$ && ${CLIENT_EXISTS} == '0' ]]; do
		...
		CLIENT_EXISTS=$(grep -w $VPN_USER /var/lib/akbarvpn/data-user-l2tp | wc -l)
...
echo -e "### $VPN_USER $exp">>"/var/lib/akbarvpn/data-user-l2tp"
cat <<EOF
